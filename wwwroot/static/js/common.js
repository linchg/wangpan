...
$(document).ready(function() {
		...
		jQuery(this).hover(
			function(){
				...
				mouseover_tid[index] = setTimeout(function() {
					jQuery(_self).addClass('hover_style');
					//jQuery(_self).find('ul.son').slideDown(200);
					jQuery(_self).find('.son').show();
				}, 100);
			},
			function(){
				var _self = this;
				...
				mouseout_tid[index] = setTimeout(function() {
					jQuery(_self).removeClass('hover_style');
					//jQuery(_self).find('ul.son').slideUp(200);
					jQuery(_self).find('.son').hide();
				}, 100);
			}
		);
	});
	...
	$('#passportBox').hover(function(){
		$(this).find(".passportList").show();
	},function(){
		$(this).find(".passportList").hide();
	});

	$('#indexVipLvl').hover(function(){
		$(this).find('.levelPopInfo').show();
	},function(){
		$(this).find('.levelPopInfo').hide();
	});
	$('#9cGmaeMenu').hover(function(){
	...
	//顶部广告图
	$('#advertSmallPic').mousemove(function(){
		$('#advertBigPic').show();
		//setTimeout(function(){$('#advertBigPic').hide();},5000)
	});
	$('#advertBigPic').mouseout(function(){
		$(this).hide();
	});
});
function changeRightHight(){
	$('.right').css({height:''});
	//左右两栏高度
	var _leftH = $('.left').innerHeight();
	var _rightH = $('.right').innerHeight();
	if(_leftH>_rightH)
	{
		if( $.browser.msie == '6.0')
		{
			var _realRightH = _leftH-35;
		} else {
			var _realRightH = _leftH-15;
		}
		$('.right').css({height:_realRightH});
	} else {
